...
TestAll.prototype = {
	testCircle: function() {
		var c = lip_shapes_Primitives.circle(0,0,1);
		utest_Assert.isTrue(c(0,0) < 0,null,{ fileName : "TestAll.hx", lineNumber : 12, className : "TestAll", methodName : "testCircle"});
		utest_Assert.isTrue(c(10,10) > 0,null,{ fileName : "TestAll.hx", lineNumber : 13, className : "TestAll", methodName : "testCircle"});
		var cc = lip_shapes_Boolean.union(c,c);
		utest_Assert.isTrue(cc(0,0) < 0,null,{ fileName : "TestAll.hx", lineNumber : 15, className : "TestAll", methodName : "testCircle"});
		utest_Assert.isTrue(cc(10,10) > 0,null,{ fileName : "TestAll.hx", lineNumber : 16, className : "TestAll", methodName : "testCircle"});
	}
	,testIndexing: function() {
		var grid = new lip_Grid3(3,5,7);
		var tests = [{ linear : 0, row : 0, col : 0, layer : 0},{ linear : 1, row : 1, col : 0, layer : 0},{ linear : 3, row : 0, col : 1, layer : 0},{ linear : 15, row : 0, col : 0, layer : 1},{ linear : 16, row : 1, col : 0, layer : 1},{ linear : 19, row : 1, col : 1, layer : 1},{ linear : 104, row : 2, col : 4, layer : 6}];
		var _g = 0;
		while(_g < tests.length) {
			var test = tests[_g];
			++_g;
			utest_Assert.equals(test.row,test.linear % grid.rows,"expected " + test.row + " for row but got " + test.linear % grid.rows + ": " + Std.string(test),{ fileName : "TestAll.hx", lineNumber : 32, className : "TestAll", methodName : "testIndexing"});
			var linear = test.linear;
			var temp = linear - grid.rows * grid.cols * Math.floor(linear / (grid.rows * grid.cols));
			var linear1 = test.linear;
			var temp1 = linear1 - grid.rows * grid.cols * Math.floor(linear1 / (grid.rows * grid.cols));
			utest_Assert.equals(test.col,Math.floor(temp / grid.rows),"expected " + test.col + " for col but got " + Math.floor(temp1 / grid.rows) + ": " + Std.string(test),{ fileName : "TestAll.hx", lineNumber : 33, className : "TestAll", methodName : "testIndexing"});
			utest_Assert.equals(test.layer,Math.floor(test.linear / (grid.rows * grid.cols)),"expected " + test.layer + " for layer but got " + Math.floor(test.linear / (grid.rows * grid.cols)) + ": " + Std.string(test),{ fileName : "TestAll.hx", lineNumber : 34, className : "TestAll", methodName : "testIndexing"});
			utest_Assert.equals(test.linear,test.layer * grid.cols * grid.rows + test.col * grid.rows + test.row,"expected " + test.linear + " for linear but got " + (test.layer * grid.cols * grid.rows + test.col * grid.rows + test.row) + ": " + Std.string(test),{ fileName : "TestAll.hx", lineNumber : 35, className : "TestAll", methodName : "testIndexing"});
		}
	}
	,__class__: TestAll
};
...
var lip_Grid3 = function(rows,cols,layers) {
	...
	while(_g2 < _g1) {
		var i = _g2++;
		_g.push(lip_PointStatus.Unknown);
	}
	this.array = _g;
	this.cursor = 0;
};
...
lip_shapes_Boolean.union = function(a,b) {
	return function(xx,yy) {
		return Math.min(a(xx,yy),b(xx,yy));
	};
};
lip_shapes_Boolean.intersection = function(a,b) {
	return function(xx,yy) {
		return Math.max(a(xx,yy),b(xx,yy));
	};
};
lip_shapes_Boolean.difference = function(a,b) {
	return function(xx,yy) {
		return Math.max(a(xx,yy),-b(xx,yy));
	};
};
lip_shapes_Boolean.complement = function(a) {
	return function(xx,yy) {
		return -a(xx,yy);
	};
};
lip_shapes_Boolean.union3 = function(a,b) {
	return function(xx,yy,zz) {
		return Math.min(a(xx,yy,zz),b(xx,yy,zz));
	};
};
var lip_shapes_Primitives = function() { };
lip_shapes_Primitives.__name__ = ["lip","shapes","Primitives"];
lip_shapes_Primitives.circle = function(x,y,r) {
	return function(xx,yy) {
		var xdiff = xx - x;
		var ydiff = yy - y;
